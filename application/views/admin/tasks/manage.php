...
function applyStyles() {
  $('#DataTables_Table_0 td').each(function() {
    ...
    const hasChildLinkWithImage = $(this).find('a:has(> img)').length;

    if (hasChildLinkWithImage && !$(this).hasClass('wrapped')) {
      $(this).addClass('wrapped');
      $(this).wrapInner('<div class="flex"></div>');
    }
...
const targetNode = document.getElementById('DataTables_Table_0');
