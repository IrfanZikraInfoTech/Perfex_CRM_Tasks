                        ...
                        <!-- Kudos Feed (Example) -->
                          ...
                          <div id="kudosfeed" class="space-y-6 data-block overflow-y-auto scrollbar-hide h-[400px] rounded-[30px]">
                            ...
                            <?php foreach($kudos_data as $kudos): 
                                  ...
                                  $seen_by_current_user = in_array($this->session->userdata('staff_user_id'), explode(',', $kudos['seen_by']));
                                  $fill_color = $seen_by_current_user ? 'text-green-500' : 'text-gray-500';
                                  ...
                                  $interval = $currentDateTime->diff($postDateTime);
                                        ...
                                        <div class="p-8 bg-white rounded-[30px] shadow-lg mb-5"   data-type="<?php 
                                            ...
                                            <div class="flex items-center justify-between mb-2">
                                                ...
                                                <button class="focus:outline-none seen-btn <?= $fill_color ?>" data-kudos-id="<?php echo $kudos['id']; ?>">
                                                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style="stroke-width: 4;">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="6" d="M3 12l7 7 11-11"/>
                                                    </svg>
                                                </button>
                                            ...
                                            <div class="flex items-center justify-between space-x-2">
                                                ...
                                                <div class="flex profile-images" style="width:100px;"> <!-- adjust width as required -->
                                                    <!-- Images of people who liked this kudos -->
                                                    <?php foreach($seen_staff_ids as $staff_id): 
                                                        if(!empty($staff_id)): ?>
                                                            <div class="w-6 h-6 rounded-full overflow-x-auto scrollbar-hide">
                                                                <?= staff_profile_image($staff_id, ['w-6 h-6 rounded-full'], 'thumb') ?>
                                                            </div>
...
<script>
    ...
    var kudosId = $(this).data('kudos-id');
    var btn = $(this);
    var profileImagesDiv = btn.closest('div').next('.profile-images'); // Find the profile images div just like in the like-btn
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.success) {
                if(response.action === 'marked as seen') {
                    btn.css('color', 'green'); // Change the button color to indicate seen status

                    // Add the profile image of the user who marked it as seen
                    var newImageHtml = `<div class="w-6 h-6 rounded-full overflow-x-auto scrollbar-hide">
                                            <img src="${response.image_url}" class="w-6 h-6 rounded-full">
                                        </div>`;
                    profileImagesDiv.append(newImageHtml);
                } else if(response.message === 'Already marked as seen') {
                    alert('You have already marked this kudos as seen!');
                }
            } else {
                alert('Something went wrong!');
            }
