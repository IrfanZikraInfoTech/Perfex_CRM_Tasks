...

{
	...
	public function staff_resigned_by_month($month) {
    $this->db->select('COUNT(staffid) as total_resigned');
    $this->db->where('active', 0); // Select only inactive staff
    $this->db->where("DATE_FORMAT(date_update, '%Y-%m') =", $month); // Check the date_updated for the given month
    $result = $this->db->get(db_prefix() . 'staff')->row();

    return $result ? (int)$result->total_resigned : 0;
}
